...
# Liste des fourmis avec position, orientation et état (nourriture)
ant_array = [
    {"pos": [0, 0], "angle": (0,1), "have_food": False},
    {"pos": [4, 3], "angle": (0,1), "have_food": False}
]

# Fonction pour dessiner le tableau avec les fourmis et icônes
    ...
    for i in range(len(list)):
        ...
        for j in range(len(list[0])):
            ...
            if j == len(list[0]) - 1:  # Si c'est la dernière colonne
                print("🐜" if fourmi else charctere)  # Saut de ligne après
            else:
